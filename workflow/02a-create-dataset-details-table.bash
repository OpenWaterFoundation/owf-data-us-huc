...
Files are available containing all HUC2 basins, and each HUC2 basin in a separate file.

| **Layer File** | **Size (bytes)** |
|--|--:|" >> ${tableFile}
...
ls -1 ../data/huc2-basins* ../data/huc2-basin-* | while read filepath; do
  ...
  echo '| [`'${filename}'`]('${filename}') |' ${size} '|' >> ${tableFile} 
done
...
| **Layer File (HUC4 basin)** | **Size (bytes)** |
|--|--:|" >> ${tableFile}
...
| **Layer File (HUC8 basins in HUC4)** | **Size (bytes)** |
|--|--:|" >> ${tableFile}
...
| **Layer File (HUC8 basin)** | **Size (bytes)** |
|--|--:|" >> ${tableFile}
...
| **Layer File (HUC12 basins in HUC4)** | **Size (bytes)** |
|--|--:|" >> ${tableFile}
...
| **Layer File (HUC12 basins in HUC8)** | **Size (bytes)** |
|--|--:|" >> ${tableFile}
